GPL5175 <-
    tuberculosis.pipeline:::get_annotation_data("GPL5175") |>
    dplyr::rename_with(snakecase::to_screaming_snake_case)

by_symbol <-
    dplyr::select(GPL5175, ID, GENE_ASSIGNMENT) |>
...
by_ensembl_gene_id <-
    dplyr::select(GPL5175, ID, MRNA_ASSIGNMENT) |>
    dplyr::filter(!(ID %in% remapped_ids)) |>
...
by_ena <-
    dplyr::select(GPL5175, ID, GB_LIST) |>
    dplyr::filter(!(ID %in% remapped_ids)) |>
...
usethis::use_data(GPL5175, overwrite = TRUE)
