GPL96 <-
    tuberculosis.pipeline:::get_annotation_data("GPL96") |>
    dplyr::rename_with(snakecase::to_screaming_snake_case)

by_symbol <-
    dplyr::select(GPL96, ID, GENE_SYMBOL) |>
...
by_entrez_id <-
    dplyr::select(GPL96, ID, ENTREZ_GENE_ID) |>
    dplyr::filter(!(ID %in% remapped_ids)) |>
...
usethis::use_data(GPL96, overwrite = TRUE)
