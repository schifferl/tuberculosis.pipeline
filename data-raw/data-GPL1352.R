GPL1352 <-
    tuberculosis.pipeline:::get_annotation_data("GPL1352") |>
    dplyr::rename_with(snakecase::to_screaming_snake_case)

by_symbol <-
    dplyr::select(GPL1352, ID, GENE_SYMBOL) |>
...
by_entrez_id <-
    dplyr::select(GPL1352, ID, ENTREZ_GENE_ID) |>
    dplyr::filter(!(ID %in% remapped_ids)) |>
...
by_ena <-
    dplyr::select(GPL1352, ID, GB_ACC) |>
    dplyr::filter(!(ID %in% remapped_ids)) |>
...
usethis::use_data(GPL1352, overwrite = TRUE)
