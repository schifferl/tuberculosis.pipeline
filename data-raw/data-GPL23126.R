GPL23126 <-
    tuberculosis.pipeline:::get_annotation_data("GPL23126") |>
    dplyr::rename_with(snakecase::to_screaming_snake_case)

by_symbol <-
    dplyr::select(GPL23126, ID, GENE_ASSIGNMENT) |>
...
by_ensembl_gene_id <-
    dplyr::select(GPL23126, ID, MRNA_ASSIGNMENT) |>
    dplyr::filter(!(ID %in% remapped_ids)) |>
...
usethis::use_data(GPL23126, overwrite = TRUE)
