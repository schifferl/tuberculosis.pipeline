GPL6947 <-
    tuberculosis.pipeline:::get_annotation_data("GPL6947") |>
    dplyr::rename_with(snakecase::to_screaming_snake_case)

by_symbol <-
    dplyr::select(GPL6947, ID, SYMBOL) |>
...
by_entrez_id <-
    dplyr::select(GPL6947, ID, ENTREZ_GENE_ID) |>
    dplyr::filter(!(ID %in% remapped_ids)) |>
...
by_ena <-
    dplyr::select(GPL6947, ID, GB_ACC) |>
    dplyr::filter(!(ID %in% remapped_ids)) |>
...
usethis::use_data(GPL6947, overwrite = TRUE)
