GPL6884 <-
    tuberculosis.pipeline:::get_annotation_data("GPL6884") |>
    dplyr::rename_with(snakecase::to_screaming_snake_case)

by_symbol <-
    dplyr::select(GPL6884, ID, SYMBOL) |>
...
by_entrez_id <-
    dplyr::select(GPL6884, ID, ENTREZ_GENE_ID) |>
    dplyr::filter(!(ID %in% remapped_ids)) |>
...
by_ena <-
    dplyr::select(GPL6884, ID, GB_ACC) |>
    dplyr::filter(!(ID %in% remapped_ids)) |>
...
by_refseq_accession <-
    dplyr::select(GPL6884, ID, REF_SEQ_ID) |>
    dplyr::filter(!(ID %in% remapped_ids)) |>
...
usethis::use_data(GPL6884, overwrite = TRUE)
