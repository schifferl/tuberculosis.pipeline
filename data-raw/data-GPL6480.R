GPL6480 <-
    tuberculosis.pipeline:::get_annotation_data("GPL6480") |>
    dplyr::rename_with(snakecase::to_screaming_snake_case)

by_symbol <-
    dplyr::select(GPL6480, ID, GENE_SYMBOL) |>
...
by_ensembl_gene_id <-
    dplyr::select(GPL6480, ID, ENSEMBL_ID) |>
    dplyr::filter(!(ID %in% remapped_ids)) |>
...
by_ena <-
    dplyr::select(GPL6480, ID, GB_ACC) |>
    dplyr::filter(!(ID %in% remapped_ids)) |>
...
by_refseq_accession <-
    dplyr::select(GPL6480, ID, REFSEQ) |>
    dplyr::filter(!(ID %in% remapped_ids)) |>
...
usethis::use_data(GPL6480, overwrite = TRUE)
