GPL6102 <-
    tuberculosis.pipeline:::get_annotation_data("GPL6102") |>
    dplyr::rename_with(snakecase::to_screaming_snake_case)

by_symbol <-
    dplyr::select(GPL6102, ID, SYMBOL) |>
...
by_entrez_id <-
    dplyr::select(GPL6102, ID, ENTREZ_GENE_ID) |>
    dplyr::filter(!(ID %in% remapped_ids)) |>
...
by_ena <-
    dplyr::select(GPL6102, ID, GB_ACC) |>
    dplyr::filter(!(ID %in% remapped_ids)) |>
...
by_refseq_accession <-
    dplyr::select(GPL6102, ID, REF_SEQ_ID) |>
    dplyr::filter(!(ID %in% remapped_ids)) |>
    dplyr::mutate(REF_SEQ_ID = stringr::str_remove_all(REF_SEQ_ID, "\\.[0-9]+$")) |>
    dplyr::filter(!base::is.na(REF_SEQ_ID)) |>
    dplyr::mutate(SYMBOL = tuberculosis.pipeline:::map_refseq_accession(REF_SEQ_ID)) |>
    dplyr::filter(!base::is.na(SYMBOL)) |>
    dplyr::select(ID, SYMBOL)
...
usethis::use_data(GPL6102, overwrite = TRUE)
