GPL4133 <-
    tuberculosis.pipeline:::get_annotation_data("GPL4133") |>
    dplyr::rename_with(snakecase::to_screaming_snake_case) |>
    dplyr::rename(SPOT_ID = SPOT_ID_5)

by_symbol <-
    dplyr::select(GPL4133, SPOT_ID, GENE_SYMBOL) |>
...
by_ensembl_gene_id <-
    dplyr::select(GPL4133, SPOT_ID, ENSEMBL_ID) |>
    dplyr::filter(!(SPOT_ID %in% remapped_ids)) |>
...
by_entrez_id <-
    dplyr::select(GPL4133, SPOT_ID, GENE) |>
    dplyr::filter(!(SPOT_ID %in% remapped_ids)) |>
...
by_ena <-
    dplyr::select(GPL4133, SPOT_ID, GB_ACC) |>
    dplyr::filter(!(SPOT_ID %in% remapped_ids)) |>
...
by_refseq_accession <-
    dplyr::select(GPL4133, SPOT_ID, REFSEQ) |>
    dplyr::filter(!(SPOT_ID %in% remapped_ids)) |>
...
usethis::use_data(GPL4133, overwrite = TRUE)
